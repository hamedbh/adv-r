...
fixed_point <- function(f, 
    ...
    for (i in seq_len(n_max)) {
        print(paste0("Guess ", i, " is ", current_guess))
        if (abs((f(current_guess) - current_guess)) < tol) {
...
```{r}
# find fixed point of x^x
fixed_point(function(x) log(1000)/log(x), 5)
```

Now build a version of `fixed_point` with average damping.

```{r}
fixed_point_damped <- function(f, 
                        init, 
                        n_max = 10000, 
                        tol = 0.00001) {
    current_guess <- init
    for (i in seq_len(n_max)) {
        ...
        if (abs((f(current_guess) - current_guess)) < tol) {
            ...
            current_guess <- mean(f(current_guess), current_guess)
        }
    }
}
```

How quickly can this find fixed-point of $x^x$?

```{r}
fixed_point_damped(function(x) log(1000)/log(x), 5)
```

Version of truncated continued fraction function using recursion.
...
Now calculate golden ratio to 4 decimal places and check which value of $k$ first gets it right.

```{r}
tests <- map_dbl(1:20, 
                 ~ round(cont_frac(rep(1, .x), rep(1, .x), .x), 
                         digits = 4))

detect_index(tests, ~ .x == 0.6180)
```

Now do the same but using iteration.

```{r}
cont_frac_iter <- function(n, d, k) {
    frac <- function(i, x) {
        if (i == 0) {
            return(x)
        } else {
...
Now calculate golden ratio tagain with this function

```{r}
tests_iter <- map_dbl(1:20, 
                      ~ round(cont_frac_iter(rep(1, .x), rep(1, .x), .x), 
                              digits = 4))

detect_index(tests_iter, ~ .x == 0.6180)
```
